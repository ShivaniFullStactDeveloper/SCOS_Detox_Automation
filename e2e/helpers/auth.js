...
/**
 * Verify Institute Selection screen is displayed.
 */
async function expectInstituteScreen() {
  await waitForVisible(IDS.instituteScreen, LONG_TIMEOUT);
 ...
 */
async function expectRoleScreen() {
  await waitForVisible(IDS.roleScreen, LONG_TIMEOUT);
  await expectVisible(IDS.roleHeader);
}

/**
 * Verify Dashboard screen is loaded successfully.
 */
async function expectDashboardScreen() {
  await waitForVisible(IDS.dashboard, LONG_TIMEOUT);
  await expectVisible(IDS.dashboardGreeting);
}

/**
 * Verify Logout confirmation popup is displayed.
 */
async function expectLogoutModal() {
  await waitForVisible(IDS.logoutModal, MEDIUM_TIMEOUT);
}

/**
 * Logout user if currently logged in.
 * Used for cleanup after test execution.
 */
async function logoutUser() {
  try {
    if (await isVisible(IDS.dashboard)) {
...
/**
 * Navigate from Institute screen
 * to Dashboard by selecting institute and role.
 */
async function navigateInstituteRoleToDashboard() {
  await selectFirstInstitute();
